...
program
  ...
  .action(async (options) => {
    try {
      ...
      let dataset;
      try {
        dataset = JSON.parse(await fs.readFile(datasetPath, 'utf-8'));
      } catch {
        dataset = [
